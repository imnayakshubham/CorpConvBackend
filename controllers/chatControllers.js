...
const accessChat = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (String(userId) === String(req.user._id)) {
      ...
      }).populate({ path: 'users', select: 'public_user_name username user_bio user_job_role user_job_experience user_current_company_name user_public_location avatar_config user_public_profile_pic lastActiveAt' })
...
const fetchChats = asyncHandler(async (req, res) => {
  try {
    ...
    const results = await Chat.find({
      ...
      .populate({
        path: "users",
        select: "public_user_name username user_bio user_job_role user_job_experience user_current_company_name user_public_location avatar_config user_public_profile_pic lastActiveAt"
      })
      .populate("groupAdmin")
      .populate("latestMessage")
      .sort({ updatedAt: -1 })
